...
public class DynamicFreeTypeFont {
	...
	public CharInfo addChar(char c)
	{
		...
		tface.loadChar(c,
					   fontParam.size < 16 ? FreeType.FT_LOAD_DEFAULT
										   : FreeType.FT_LOAD_NO_HINTING | FreeType.FT_LOAD_NO_BITMAP
		);
		...
		TextureRegion textureRegion = new TextureRegion(curTexture, x, y, pixMapWidth, pixmap.getHeight());
		textureRegion.flip(false, true);
		x += pixMapWidth + PAD;

		GlyphMetrics metrics = slot.getMetrics();
		CharInfo ci = new CharInfo();
		ci.region = textureRegion;
	...
	private float getCharWidth(char c) {
		...
		tface.loadChar(c, fontParam.size < 16 ? FreeType.FT_LOAD_DEFAULT
											  : FreeType.FT_LOAD_NO_HINTING | FreeType.FT_LOAD_NO_BITMAP
		);
